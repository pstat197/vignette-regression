...
# Mean Squared Error
test_mse <- mean((pred - test$Life.expectancy)^2)
test_mse
```
...
The R-squared value for the ridge regression model on the testing data outputs to be **82.79%**, which is a **slight decrease in performance** compared with the linear regression model at a R-squared value of **82.90%.** The MSE for our test partition set is **12.9954**, which is slightly higher than the the test MSE of the linear regression model.
