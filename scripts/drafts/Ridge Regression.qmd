...
partion <- life_clean %>%  # Splittin the data into testing and training 
  initial_split(prop = .8)
traing <- training(partion)
train_x <- as.matrix(traing$Life.expectancy) # transforming our Xs into a matrix
traing <- traing %>%
  select(-Life.expectancy) # removing our x
train_y <- as.matrix(traing) # transforming our Ys into a matrix

test <- testing(partion)
